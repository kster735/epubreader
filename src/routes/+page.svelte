<script lang="ts">
	...
	let title: String;
	let selectBook = {num: 0};
	let books = ['Hyperion-Dan Simmons.epub', 'Dune-FrankHerbert.epub', 'SpeakerforDead-OrsonScottCard.epub'];	
	...
	onMount(() => {
		...
		// book = ePub('Hyperion-Dan Simmons.epub');
		book = ePub(books[selectBook.num]);
		// book = ePub('Hyperion-Dan Simmons.epub');
		// book = ePub('Hyperion-Dan Simmons.epub');
		// book = ePub("Madame-GustaveFlaubert.epub");
...
<div class="container">
	...
	<div class="header">
		<ReadingBar bind:contentsShow {title} bind:selectBook/>
	</div>
